...
`gowin/minc/` is a Gowin IDE project (for an actual FPGA build/synthesis) wrapping the core in `gowin/minc/src/minc_gw_top.sv`, which adds a Gowin block-RAM instance plus memory-mapped peripherals: a UART (`uart_master/`) and an I2C master (`i2c_master/`, mapped at `0x0010`-`0x0013`). `UART`, `PORTA`, `WAIT` (wait-state support), and `I2C` are toggled via `` `define `` in `minc_gw_top.sv` — enabling `UART` auto-enables `WAIT` since the UART core needs wait-state support to throttle CPU memory access. `gowin/ips/timer/timer8.sv` is a standalone 8-bit interval timer IP (with its own `timer8_tb.sv` testbench) that exists in the tree but is not yet instantiated in `minc_gw_top.sv`.
