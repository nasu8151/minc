...
**`verilog/minc_h.sv` is the current, actively-developed CPU implementation** (18-bit instruction word, 4-state `S_FETCH → S_DECEXEC → S_MA → S_WB` pipeline, memory-mapped SP at addresses `0x0000`/`0x0001`). `verilog/minc.sv` is an older/legacy core (15-bit instruction word, different opcode map and state machine) — check which one a task actually targets before editing; tests and simulation (`minc_tb.sv`, `tests/testfuncs.py`) build against `minc_h.sv`.
